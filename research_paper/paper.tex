...
In our experiments (averaged over 10 seeds), the PID controller achieved faster convergence to the safety constraint and reduced oscillation in the penalty term. Specifically, the GA baseline exhibited a "sawtooth" pattern in $\lambda_t$, leading to unstable utility scores (variance $\sigma^2_{GA} = 0.15$ vs $\sigma^2_{PID} = 0.04$). The integral term ($K_I$) proved crucial for eliminating steady-state error when the agent operated near the safety boundary.
