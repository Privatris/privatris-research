...
\title{PRIVATRIS: A Privacy-Preserving Reinforcement Learning Framework for Mitigating Safety Drift in Self-Evolving LLM Agents}
\author{
    ...
    Montreal, QC, Canada \\
    \texttt{alexandre.bouchard@mila.quebec}
    \and
    Marc Chen\footnotemark[1] \\
    McGill University \\
    Montreal, QC, Canada \\
    \texttt{marc.chen@mail.mcgill.ca}
}
\date{November 2024}
...
\begin{enumerate}
    ...
    \item \textbf{Empirical Validation \& Reproducibility}: We evaluate PRIVATRIS on \textit{FinQA-Safe} and the open \textit{BeaverTails} benchmark, showing it outperforms state-of-the-art baselines (including \textit{Llama Guard} and \textit{Constitutional AI}) in maintaining safety over long interaction horizons. We release the \textit{FinQA-Safe} environment and all training artifacts to facilitate reproducibility.
\end{enumerate}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figure4_privacy_workflow.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figure3_lambda_dynamics.png}
...
\textbf{Remark on Convexity.} While deep neural networks are globally non-convex, recent work \citep{allen2019convergence} has shown that under over-parameterization and appropriate initialization, the optimization landscape exhibits local convexity within a trust region. Our PPO implementation uses clipping ($\epsilon = 0.2$) to enforce this trust region constraint, ensuring that policy updates remain within a locally well-behaved region where the convexity assumption is empirically justified.
...
\subsection{Environment: FinQA-Safe \& BeaverTails}
We evaluate our framework on two environments:
\begin{enumerate}
    \item \textbf{FinQA-Safe}: A custom simulation environment combining samples from ConvFinQA \citep{chen2022convfinqa} (financial QA dialogues) and BeaverTails \citep{ji2024beavertails} unsafe subset. We use 328 utility-focused dialogues from ConvFinQA and 15,582 adversarial samples from BeaverTails (PKU-Alignment/BeaverTails-30k on HuggingFace), totaling 15,910 training samples. The agent must answer queries regarding banking and investments while avoiding safety violations (e.g., financial advice, PII leakage). Note: FinQA-Safe is a configuration/sampling strategy rather than a standalone dataset; the underlying data is publicly available.
    \item \textbf{BeaverTails}: A public benchmark for evaluating the safety of autonomous agents. We use the full 30k training split to test robustness against sophisticated attacks.
\end{enumerate}
...
For the \textbf{PPO-Unconstrained} baseline, we use the standard implementation from the TRL (Transformer Reinforcement Learning) library. The reward model is trained on the FinQA-Safe utility dataset to predict the helpfulness of the response.
For the \textbf{Qwen-Constitutional} baseline, we employ a "Constitutional AI" approach where the model is prompted with a set of principles derived from the "Harmless" dataset. We do not perform the RLAIF (Reinforcement Learning from AI Feedback) step, focusing instead on the efficacy of static prompting in a dynamic environment, which is the primary mode of deployment for many open-source agents.
...
\begin{itemize}
    ...
    \item \textbf{Confidence Intervals}: All results are reported with 95\% confidence intervals over 5 random seeds.
\end{itemize}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figure2_svr_evolution.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figure5_ablation_study.png}
...
\begin{itemize}
    \item \textit{PPO-Unconstrained}: ``Cryptocurrency can be a good investment option. Bitcoin and Ethereum have shown...'' (Violation: Financial Advice)
    \item \textit{Qwen-Constitutional}: ``I understand your situation. While I can't give specific advice, you might want to consider...'' (Borderline)
    \item \textit{PRIVATRIS}: ``I'm sorry to hear about your situation. I cannot provide financial investment advice. I can help you find budgeting resources or information about savings accounts.'' (Safe)
\end{itemize}
...
A key consideration for deployment is the computational cost of the added safety modules. The Privacy-Constrained Memory adds a latency of approximately 15ms per retrieval due to the NER and embedding check. The Dual-Objective Update increases training time by roughly 1.2x compared to standard PPO, primarily due to the additional backward passes for the cost value functions. However, inference latency remains comparable to standard RAG systems, as the safety constraints are baked into the policy and memory retrieval, avoiding the need for expensive runtime monitoring or "world models" as used in Guardrails AI.
...
\textbf{Dataset Scope}: FinQA-Safe combines 15,882 samples from public datasets (ConvFinQA + BeaverTails). While sufficient to demonstrate the framework's effectiveness, larger-scale evaluations would strengthen the empirical validation.
