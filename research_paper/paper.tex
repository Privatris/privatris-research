...
\textbf{Theorem 2.} \textit{The Privacy-Constrained Memory satisfies $\epsilon$-local differential privacy with respect to the user's identity, provided the NER recall rate is $R > 1-\delta$ and the embedding noise is calibrated to the sensitivity of the embedding function.}

\textit{Proof Sketch.} Let $\mathcal{M}$ be the memory mechanism. For two neighboring interaction histories $D, D'$ differing by one user's PII, the probability ratio $P(\mathcal{M}(D) \in S) / P(\mathcal{M}(D') \in S)$ is bounded by $e^\epsilon$. The NER step acts as a randomized response mechanism with failure probability $\delta$. The subsequent embedding check adds a second layer of protection. If the NER fails (probability $\delta$), the embedding constraint rejects vectors close to sensitive clusters, effectively truncating the privacy loss. Thus, the total leakage is bounded by $\epsilon$ with probability $1-\delta$.
...
\begin{enumerate}
    \item \textbf{FinQA-Safe}: A simulation environment for financial customer service combining ConvFinQA (300 utility-focused dialogues) and BeaverTails unsafe subset (15,582 adversarial samples). The agent must answer queries regarding banking and investments while avoiding safety violations (e.g., financial advice, PII leakage).
    \item \textbf{BeaverTails}: A public benchmark for evaluating the safety of autonomous agents in long-horizon tasks. We use the ``Jailbreak'' subset to test robustness against sophisticated attacks.
\end{enumerate}
...
\bottomrule
\end{tabular}
\caption{Comparison of Utility and Safety Violation Rates (SVR) across models.}
\label{tab:results}
\end{table}
